...
2. **架构层**：借鉴 **Claude / Claude Code** 的意图驱动思路——先做用户意图识别并区分简单与复杂任务；调研 **ReAct** 与 **Plan-ReSolve** 后，按可观测与可审计目标选型；落地 **多 Agent 编排**（主 Agent 汇总、质疑 Agent 校准）与 **SSE 事件化**。代码落地上 **`api/routers` 做薄入口（协议与校验）**；**`agents/*` 承载编排与 LLM 策略**；**`common/*`** 做模型、工具、流式、数据库横切；**`lifespan`** 统一拉起 **APScheduler**。  
